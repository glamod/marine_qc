...
class SpeedChecker:
    ...
    def valid_parameters(self) -> bool:
        ...
        if not (self.speed_limit >= 0):
            warnings.warn(f"Invalid speed_limit: {self.speed_limit}. Must be zero or positive.", UserWarning, stacklevel=2)
        elif not (self.min_win_period >= 0):
            warnings.warn(f"Invalid speed_limit: {self.min_win_period}. Must be zero or positive.", UserWarning, stacklevel=2)
        elif not (self.min_win_period >= 0):
            warnings.warn(f"Invalid speed_limit: {self.min_win_period}. Must be zero or positive.", UserWarning, stacklevel=2)
        elif not (self.max_win_period >= self.min_win_period):
            warnings.warn("max_win_period must be greater than or equal to min_win_period.", UserWarning, stacklevel=2)
        else:
            valid = True
    ...
    def valid_arrays(self) -> bool:
        ...
        if any(np.isnan(self.lon)):
            warnings.warn("Nan(s) found in longitude.", UserWarning, stacklevel=2)
        elif any(np.isnan(self.lat)):
            warnings.warn("Nan(s) found in latitudes.", UserWarning, stacklevel=2)
        elif any(np.isnan(self.hrs)):
            warnings.warn("Nan(s) found in time differences.", UserWarning, stacklevel=2)
        elif not is_monotonic(self.hrs):
            warnings.warn("times are not sorted: {self.hrs}", UserWarning, stacklevel=2)
        else:
            valid = True
    ...
    * max_win_period: maximum period of time in days over which position is assessed for no movement (this should be
    ...
    def valid_parameters(self) -> bool:
        ...
        if self.smooth_win < 1:
            warnings.warn("Invalid smooth_win: {self.smooth_win}. Must be positive.", UserWarning, stacklevel=2)
        elif self.smooth_win % 2 == 0:
            warnings.warn("Invalid smooth_win: {self.smooth_win}. Must be an odd number.", UserWarning, stacklevel=2)
        elif self.min_win_period < 1:
            warnings.warn(f"Invalid min_win_period: {self.min_win_period}. Must be positive.", UserWarning, stacklevel=2)
        elif self.max_win_period is None:
            valid = True
        elif self.max_win_period < 1:
            warnings.warn("Invalid max_win_period: {self.max_win_period}. Must be positive.", UserWarning, stacklevel=2)
...
class SSTTailChecker:
    ...
    def valid_parameters(self):
        ...
        if self.long_win_len < 1:
            warnings.warn("Invalid long_win_len: {self.long_win_len}. Must be positive.", UserWarning, stacklevel=2)
        elif not (self.long_win_len % 2 != 0):
            warnings.warn("Invalid long_win_len: {self.long_win_len}. Must be an odd number.", UserWarning, stacklevel=2)
        elif self.long_err_std_n < 0:
            warnings.warn("Invalid long_err_std_n: {self.long_err_std_n}. Must be zero or positive.", UserWarning, stacklevel=2)
        elif self.short_win_len < 1:
            warnings.warn("Invalid short_win_len: {self.short_win_len}. Must be positive.", UserWarning, stacklevel=2)
        elif self.short_err_std_n < 0:
            warnings.warn("Invalid short_err_std_n: {self.short_err_std_n}. Must be zero or positive.", UserWarning, stacklevel=2)
        elif self.short_win_n_bad < 1:
            warnings.warn("Invalid short_win_n_bad: {self.short_win_n_bad}. Must be positive.", UserWarning, stacklevel=2)
        elif self.drif_inter < 0:
            warnings.warn("Invalid drif_inter: {self.drif_inter}. Must be zero or positive.", UserWarning, stacklevel=2)
        elif self.drif_intra < 0:
            warnings.warn("Invalid drif_intra: {self.drif_intra}. Must be zero or positive.", UserWarning, stacklevel=2)
        elif self.background_err_lim < 0:
            warnings.warn("Invalid background_err_lim: {self.background_err_lim}. Must be zero or positive.", UserWarning, stacklevel=2)
        else:
            valid = True
    ...
    def do_sst_tail_check(self, start_tail: bool):
        ...
        if not is_monotonic(self.hrs):
            warnings.warn("Times do not increase monotonically", UserWarning, stacklevel=2)
            self.qc_outcomes[:] = untestable
            return
    ...
    @staticmethod
    def _parse_rep(lat, lon, ostia, ice, bgvar, dates) -> tuple[float, float, float, bool]:
        """
        Process a report
        ...
        if ice < 0.0 or ice > 1.0:
            warnings.warn("Invalid ice value", UserWarning, stacklevel=2)
            invalid_ob = True
        ...
        for ind in range(self.nreps):
            ...
            if good_match:
                if bg_val < -5.0 or bg_val > 45.0:
                    warnings.warn("Background value is invalid", UserWarning, stacklevel=2)
                    invalid_series = True
                if bgvar_val < 0 or bgvar_val > 10.0:
                    warnings.warn("Background variance is invalid", UserWarning, stacklevel=2)
                    invalid_series = True
...
class SSTBiasedNoisyChecker:
    ...
    def valid_parameters(self) -> bool:
        ...
        valid = False
        if self.n_eval < 1:
            warnings.warn("Invalid n_eval: {self.n_eval}. Must be positive.", stacklevel=2)
        elif self.bias_lim < 0:
            warnings.warn("Invalid bias_lim: {self.bias_lim}. Must be zero or positive.", stacklevel=2)
        elif self.drif_inter < 0:
            warnings.warn("Invalid drif_inter: {self.drif_inter}. Must be zero or positive.", stacklevel=2)
        elif self.drif_intra < 0:
            warnings.warn("Invalid drif_intra: {self.drif_intra}. Must be zero or positive.", stacklevel=2)
        elif self.err_std_n < 0:
            warnings.warn("Invalid err_std_n: {self.err_std_n}. Must be zero or positive.", stacklevel=2)
        elif self.n_bad < 1:
            warnings.warn("Invalid n_bad: {self.n_bad}. Must be positive.", stacklevel=2)
        elif self.background_err_lim < 0:
            warnings.warn("Invalid background_err_lim: {self.background_err_lim}. Must be zero or positive.", stacklevel=2)
        else:
            valid = True
    ...
    def _preprocess_reps(self) -> bool:
        ...
        if not is_monotonic(self.hrs):
            warnings.warn("Not sorted in time order", UserWarning, stacklevel=2)
            invalid_series = True
