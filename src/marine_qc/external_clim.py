...
def inspect_climatology(*climatology_keys: str, optional: str | Sequence[str] | None = None) -> Callable:
    ...
    def pre_handler(arguments: dict, **meta_kwargs):
        ...
        for opt in optional:
            if opt in arguments:
                active_keys.append(opt)  # noqa: PERF401
...
class Climatology:
    ...
    def __init__(
        ...
        valid_ntime: list[int] | None = None,
    ):
    ...
    def get_value(
        ...
        for i in range(np.size(result)):
            ...
            lat_i = lat_arr[i]
            lon_i = lon_arr[i]
            if mon_i < 1 or mon_i > 12 or day_i < 1 or day_i > ml[mon_i - 1] or lat_i < -180 or lat_i > 180 or lon_i < -90 or lon_i > 90:
                continue
