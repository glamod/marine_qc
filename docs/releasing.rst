...
.. warning::
    ...
    It is important to be aware that any changes to files found within the ``src/marine_qc`` folder (with the exception of ``src/marine_qc/__init__.py``) will trigger the ``bump-version.yml`` workflow. Be careful not to commit changes to files in this folder when preparing a new release.
...
The new version based off of the version checked out will now be available via `pip` (`pip install marine_qc`).
...
From the marine_qc source folder we can enter into the docker container, providing access to the `src/marine_qc` source files by linking them to the running image:

    .. code-block:: console

        sudo docker run --rm -ti -v $(pwd):/src/marine_qc -w /src/marine_qc quay.io/pypa/manylinux_2_24_x86_64 bash
...
This will then place two files in `marine_qc/dist/` ("marine_qc-1.2.3-py3-none-any.whl" and "marine_qc-1.2.3.tar.gz").
